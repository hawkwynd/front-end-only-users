...
function Add_Edit_User() {
	...
	if (!isset($_POST['Email_Confirmed'])) {$_POST['Email_Confirmed'] = null;}
	...
	if (!isset($_POST['Third_Party_ID'])) {$_POST['Third_Party_ID'] = null;}
	...
	if ($_POST['Third_Party_ID'] != null) {$User_Fields['User_Third_Party_ID'] = $_POST['Third_Party_ID'];}
	...
	elseif (isset($_POST['level']) and $Allow_Level_Choice == "Yes") {$User_Fields['Level_ID'] = $_POST['level'];}
	...
	if ($_POST['Email_Confirmation'] == "Yes") {$User_Fields['User_Email_Confirmed'] = "Yes";}
	if ($_POST['Email_Confirmation'] == "No") {$User_Fields['User_Email_Confirmed'] = "No";}
	if ($_POST['User_Membership_Fees_Paid'] == "Yes") {$User_Fields['User_Membership_Fees_Paid'] = "Yes";}
	if ($_POST['User_Membership_Fees_Paid'] == "No") {$User_Fields['User_Membership_Fees_Paid'] = "No";}
...
function EWD_FEUP_Send_Email($User_Fields, $Additional_Fields_Array, $User_ID = 0) {
	...
	if (!isset($User_Fields['Username'])) {$User_Fields['Username'] = $wpdb->get_var($wpdb->prepare("SELECT Username FROM $ewd_feup_user_table_name WHERE User_ID=%d", $User_ID));}
	if (!isset($User_Fields['User_Date_Created'])) {$User_Fields['User_Date_Created'] = $wpdb->get_var($wpdb->prepare("SELECT User_Date_Created FROM $ewd_feup_user_table_name WHERE User_ID=%d", $User_ID));}
...
function EWD_FEUP_Send_Admin_Registration_Email($User_Fields, $Additional_Fields_Array, $User_ID) {
	...
	if (trim($New_Registration_Email_to_Admin) == "") {
		...
		$Message_Body = $New_Registration_Email_to_Admin;
		$Message_Body = str_replace("[user-id]", $User_ID, $Message_Body);
		$Message_Body = str_replace("[username]", $User_Fields['Username'], $Message_Body);
		$Fields = $wpdb->get_results("SELECT Field_ID, Field_Name FROM $ewd_feup_fields_table_name");
		foreach ($Fields as $Field) {
			$Field_Value = $wpdb->get_var($wpdb->prepare("SELECT Field_Value FROM $ewd_feup_user_fields_table_name WHERE Field_ID=%d AND User_ID=%d", $Field->Field_ID, $User_ID));
			$Message_Body = str_replace("[" . $Field->Field_Name . "]", $Field_Value, $Message_Body);
		}
	}
...
function EWD_FEUP_Send_Admin_Approval_Email($User_Fields, $Additional_Fields_Array, $User_ID = 0) {
	...
	$Email_Field = str_replace(" ", "_", $Email_Field);
	if($Username_Is_Email == "Yes") {
		$User_Email = $wpdb->get_var($wpdb->prepare("SELECT Username FROM $ewd_feup_user_table_name WHERE User_ID=%d", $User_ID));
		$User_Fields['Username'] = $User_Email;
	} else {
		$User_Email = $Additional_Fields_Array[$Email_Field]['Field_Value'];
	}
...
function Mass_Approve_EWD_FEUP_Users() {
	...
	if (is_array($Users)) {
		foreach ($Users as $User) {
			$Current_User = $wpdb->get_row($wpdb->prepare("SELECT * FROM $ewd_feup_user_table_name WHERE User_ID='%d'", $User_ID));
			...
			if ($Current_User->User_Admin_Approved == "No" and $Email_On_Admin_Approval == "Yes") {
...
function Add_Edit_Level() {
	...
	$Level_Include_Fields = $_POST['Field_IDs'];
