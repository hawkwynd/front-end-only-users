...
function Forgot_Password() {
	...
	if( !empty( $User ) and $Validate_Captcha == "Yes")
	{
		...
		$message .= __("Somebody requested a password reset for you. If this wasn't you, you can ignore and delete this e-mail.", 'EWD_FEUP')."\n\n";
	...
	elseif ($_POST['ewd-feup-reveal-no-username'] == "Yes") {
		return __("There is no user listed for the username/email address you entered in the database. Please go to the registration page to sign up.", 'EWD_FEUP');
	}
	else
	{
...
function Get_User_Search_Results($search_logic, $display_field) {
	...
	if (sizeOf($list) < 2) {
		...
		if ($search_logic == "AND") {$UserIDs = call_user_func_array('array_intersect',$list);}
			...
			if (is_array($Combined_IDs)) {$UserIDs = array_unique($Combined_IDs);}
		}
	}
