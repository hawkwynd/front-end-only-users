...
function Insert_Register_Form($atts) {
	...
	if ($feup_Label_Level_Select_Message == "") {$feup_Label_Level_Select_Message = __("Select a Level to Sign Up", 'EWD_FEUP');}
	...
	if (!isset($ConfirmationSuccess)) {	
		...
		if ($Registration_Type == "Facebook" or $Registration_Type == "Twitter") {
			...
			if ($Username_Is_Email == "Yes") {
				...
				else {$ReturnString .= "<input type='text' class='ewd-feup-text-input' name='Username' placeholder='" . 	$feup_Label_Username . "...' required>";}
			}
			$ReturnString .= "</div>";
			$ReturnString .= "<div class='feup-pure-control-group'>";
		...
		foreach ($Fields as $Field) {
			...
			if ($Field->Field_Type == "label") {
				...
				$ReturnString .= "<label for='" . $Field->Field_Name . "' id='ewd-feup-register-" . $Field->Field_ID . "' class='ewd-feup-field-label'>" . __($Field->Field_Name, 'EWD_FEUP'). ":" . $Req_Symbol . "</label>";
			}
