...
function EWD_FEUP_Account_Payment($atts) {
	...
	//Create form with link to pay for membership
		...
		if ($Payment_Gateway == "PayPal" or $Payment_Gateway == "") {
		...
		elseif ($Payment_Gateway == "Stripe") {
			...
			$ReturnString .= "<h2>" . __('Submit a payment of', 'EWD_FEUP') . " $" . $Payment_Amount . "</h2>";
			if (isset($_GET['payment']) && $_GET['payment'] == 'failed') {
				$ReturnString .= '<p class="failure">' . __('There was a problem with your payment, your card has not been charged.', 'EWD_FEUP') . '</p>';
			}
			$ReturnString .= "<form action='#' method='POST' id='stripe-payment-form'>";
			$ReturnString .= "<div class='form-row'>";
			$ReturnString .= "<label>" . __('Card Number', 'EWD_FEUP') . "</label>";
			$ReturnString .= "<input type='text' size='20' autocomplete='off' data-stripe='card_number'/>";
			$ReturnString .= "</div>";
			$ReturnString .= "<div class='form-row'>";
			$ReturnString .= "<label>" . __('CVC', 'EWD_FEUP') . "</label>";
			$ReturnString .= "<input type='text' size='4' autocomplete='off' data-stripe='card_cvc'/>";
			$ReturnString .= "</div>";
			$ReturnString .= "<div class='form-row'>";
			$ReturnString .= "<label>" . __('Expiration (MM/YYYY)', 'EWD_FEUP') . "</label>";
			...
			$ReturnString .= "<button type='submit' id='stripe-submit'>" . __('Submit Payment', 'EWD_FEUP') . "</button>";
			$ReturnString .= "</form>";
