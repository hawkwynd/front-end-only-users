<style>
    .radioOptions:hover{background-color: #CCC; }
</style>
...
?>
<div id="col-right" class="feup-overlfow-auto">
<div class="col-wrap">

<!-- Display a list of the users which have already been created -->
<?php
 wp_nonce_field();
 wp_referer_field();
...
$Fields = $wpdb->get_results("SELECT * FROM $ewd_feup_fields_table_name WHERE Field_Show_In_Admin='Yes' AND Field_Name IN('Name','Designation')");
...
if (isset($_REQUEST['UserSearchValue']) and $_REQUEST['UserSearchField'] == "Username") {
    ...
    if ($_REQUEST['UserSearchOperator'] == "LIKE") {
        $Sql .= " LIKE '%" . $_REQUEST['UserSearchValue'] . "%' ";
    } else {
        $Sql .= "='" . $_REQUEST['UserSearchValue'] . "' ";
    }
...
if (isset($_GET['OrderBy']) and $_GET['DisplayPage'] == "Users") {
    if ($_REQUEST['OrderBy'] == "User_Last_Login" or $_REQUEST['OrderBy'] == "User_Date_Created") {
...
if (isset($_REQUEST['UserSearchValue'])) {
    $Current_Page_With_Order_By .= "&UserSearchValue=" . $_REQUEST['UserSearchValue'] . "&UserSearchField=" . $_REQUEST['UserSearchField'] . "&UserSearchOperator=" . $_REQUEST['UserSearchOperator'];
}
?>
...
<div class="tablenav top">
    <div class="alignleft actions">
        ...
        <input type="submit" name="" id="doaction" class="button-secondary action"
               value="<?php _e('Apply', 'EWD_FEUP') ?>"/>
        <a class='feup-confirm-all-users button-secondary action'
           href='admin.php?page=EWD-FEUP-options&Action=EWD_FEUP_DeleteAllUsers&DisplayPage=Users'><?php _e("Delete All Users", 'EWD_FEUP'); ?></a>
    </div>
    <div class='tablenav-pages <?php if ($Number_of_Pages == 1) {
        echo "one-page";
    } ?>'>
        <span
            class="displaying-num"><?php echo $UserCount; ?> <?php _e("users", 'EWD_FEUP') ?> <?php if ($EWD_FEUP_Full_Version != "Yes") {
                echo "out of 100";
            } ?></span>
		<span class='pagination-links'>
			<a class='first-page <?php if ($Page == 1) {
                echo "disabled";
            } ?>' title='Go to the first page' href='<?php echo $Current_Page_With_Order_By; ?>&Page=1'>&laquo;</a>
			<a class='prev-page <?php if ($Page <= 1) {
                echo "disabled";
            } ?>' title='Go to the previous page'
               href='<?php echo $Current_Page_With_Order_By; ?>&Page=<?php echo $Page - 1; ?>'>&lsaquo;</a>
			<span class="paging-input"><?php echo $Page; ?> <?php _e("of", 'EWD_FEUP') ?> <span
                    class='total-pages'><?php echo $Number_of_Pages; ?></span></span>
			<a class='next-page <?php if ($Page >= $Number_of_Pages) {
                echo "disabled";
            } ?>' title='Go to the next page'
               href='<?php echo $Current_Page_With_Order_By; ?>&Page=<?php echo $Page + 1; ?>'>&rsaquo;</a>
			<a class='last-page <?php if ($Page == $Number_of_Pages) {
                echo "disabled";
            } ?>' title='Go to the last page'
               href='<?php echo $Current_Page_With_Order_By . "&Page=" . $Number_of_Pages; ?>'>&raquo;</a>
		</span>
    </div>
</div>
...
<table class="wp-list-table widefat tags sorttable fields-list ui-sortable" cellspacing="0">
    ...
    <tr>
        ...
        <?php if ($Admin_Approval == "Yes") { ?>
            <?php if ($_GET['OrderBy'] == "User_Admin_Approved" and $_GET['Order'] == "ASC") {
                $Order = "DESC";
            } else {
                $Order = "ASC";
            } ?>
        ...
        <?php foreach ($Fields as $Field) { ?>
            <?php if ($_GET['OrderBy'] == $Field->Field_Name and $_GET['Order'] == "ASC") {
                $Order = "DESC";
            } else {
                $Order = "ASC";
            } ?>
        ...
        <?php if ($Include_WP_Users == "Yes") { ?>
            <th scope='col' class='manage-column column-cb check-column' style="">
                <?php if ($_GET['OrderBy'] == "User_WP_ID" and $_GET['Order'] == "ASC") {
                    $Order = "DESC";
                } else {
                    $Order = "ASC";
                } ?>
        ...
        <?php } ?>
        <th scope='col' class='manage-column column-cb check-column' style="">
            <?php if ($_GET['OrderBy'] == "User_Last_Login" and $_GET['Order'] == "ASC") {
                $Order = "DESC";
            } else {
                $Order = "ASC";
            } ?>
        ...
        <th scope='col' class='manage-column column-cb check-column' style="">
            <?php if ($_GET['OrderBy'] == "User_Date_Created" and $_GET['Order'] == "ASC") {
                $Order = "DESC";
            } else {
                $Order = "ASC";
            } ?>
    ...
    if ($myrows) {
        foreach ($myrows as $User_ID) {
            ...
            foreach ($Fields as $Field) {
                ...
                echo "<td class='name column-name' nowrap>";
                if ($FieldCount == 0) {
                    echo "<strong>";
                ...
                if (strlen($User_Info->Field_Value) > 60) {
                    echo "...";
                }
                if ($FieldCount == 0) {
                    echo "</a></strong>";
            ...
            if ($Include_WP_Users == "Yes") {
                if ($User->User_WP_ID != 0) {
                    echo "<td class='name column-wpuser'>Yes</td>";
                } else {
                    echo "<td class='name column-wpuser'>No</td>";
                }
            }

            $lastLogin = strtotime($User->User_Last_Login) > 0 ? date('m/d/y', strtotime($User->User_Last_Login)) : "Never" ;
            $Created   = date('m/d/y', strtotime($User->User_Date_Created));

            echo "<td class='name column-name'>" .  $lastLogin . "</td>";
            echo "<td class='name column-name'>" . $Created . "</td>";
            echo "</tr>";
        }
    }
    ?>
...
<div id="col-left">
    <div class="col-wrap">
        ...
        <div class="form-wrap">
            ...
            <form name="newuser" id="addtag" method="post"
                  action="admin.php?page=EWD-FEUP-options&Action=EWD_FEUP_AddUser&DisplayPage=Users" class="validate"
                ...
                <?php if ($Username_Is_Email == "Yes") { ?>
                    <label for='Username' id='ewd-feup-register-username-div'
                           class='ewd-feup-field-label'><?php _e('Email', 'EWD_FEUP'); ?>: </label>
                    <input type='email' class='ewd-feup-text-input' name='Username'>
                <?php } else { ?>
                    <label for='Username' id='ewd-feup-register-username-div'
                           class='ewd-feup-field-label'><?php _e('Username', 'EWD_FEUP'); ?>: </label>
                    <input type='text' class='ewd-feup-text-input' name='Username'>
                <?php } ?>
                <label for='Password' id='ewd-feup-register-password-div'
                       class='ewd-feup-field-label'><?php _e('Password', 'EWD_FEUP') ?>: </label>
                <input type='password' class='ewd-feup-text-input ewd-feup-password-input' name='User_Password'>
                <label for='Repeat Password' id='ewd-feup-register-password-confirm-div'
                       class='ewd-feup-field-label'><?php _e('Repeat Password', 'EWD_FEUP'); ?>: </label>
                <input type='password' class='ewd-feup-text-input ewd-feup-check-password-input'
                       name='Confirm_User_Password'>
                <!--
                <label for='Password Strength' id='ewd-feup-password-strength'
                       class='ewd-feup-field-label'><?php //_e('Password Strength', 'EWD_FEUP') ?>: </label>
                <span id='ewd-feup-password-result'>Too Short</span>
                -->

                <label for='Level ID' id='ewd-feup-register-user-level-div'
                       class='ewd-feup-field-label'><?php _e('User Level', 'EWD_FEUP'); ?>: </label>
                ...
                <?php foreach ($Fields as $Field) { ?>
                    <div class="form-field form-required">
                        ...
                        <?php if ($Field->Field_Type == "text" or $Field->Field_Type == "mediumint") { ?><input
                            name="<?php echo $Field->Field_Name; ?>" id="<?php echo $Field->Field_Name; ?>" type="text"
                            value="" size="60" />
                        <?php } elseif ($Field->Field_Type == "date") { ?>
                            <input name='<?php echo $Field->Field_Name; ?>'
                                   id='ewd-feup-register-input-<?php echo $Field->Field_ID; ?>'
                                   class='ewd-feup-date-input pure-input-1-3' type='date' value=''/>
                        <?php } elseif ($Field->Field_Type == "datetime") { ?>
                            <input name='<?php echo $Field->Field_Name; ?>'
                                   id='ewd-feup-register-input-<?php echo $Field->Field_ID; ?>'
                                   class='ewd-feup-datetime-input pure-input-1-3' type='datetime-local' value=''/>
                        <?php } elseif ($Field->Field_Type == "textarea") { ?>
                            <textarea name="<?php echo $Field->Field_Name; ?>"
                                      id="<?php echo $Field->Field_Name; ?>"></textarea>
                        <?php } elseif ($Field->Field_Type == "file") { ?>
                            <input name='<?php echo $Field->Field_Name; ?>'
                                   id='ewd-feup-register-input-<?php echo $Field->Field_ID; ?>'
                                   class='ewd-feup-file-input pure-input-1-3' type='file' value=''/>
                        ...
                        <div class="radioOptions">
                            <div style="padding: 5px;font-size:12px;display:inline;"><?php echo  $Field->Field_Name; ?></div>
                            ...
                            <div style="float:right">
                                <?php foreach ($Options as $Option) { ?>
                                    <span>
                                        <input <?php echo strtoupper($Option)=="NO" ? " checked":"";?> type='radio' class='ewd-admin-small-input' name="<?php echo $Field->Field_Name; ?>" value="<?php echo strtoupper( $Option ); ?>"><?php echo strtoupper($Option); ?></span>
                                <?php } ?>
                            </div><!-- float left -->
                        </div>
                        ...
                        <?php } elseif ($Field->Field_Type == "checkbox") { ?>
                            <?php $Options = explode(",", $Field->Field_Options); ?>
                            <?php foreach ($Options as $Option) { ?><input type="checkbox" class='ewd-admin-small-input'
                                                                           name="<?php echo $Field->Field_Name; ?>[]"
                                                                           value="<?php echo $Option; ?>"><?php echo $Option; ?></br><?php } ?>
                        <?php } ?>
                        <p><?php echo $Field->Field_Description; ?></p>
                    </div>
                <?php } ?>

                <div style="text-align:right;"><input type='reset' value='Clear Permissions' name='reset' class="button-primary" ></div>

                <hr>

                <p class="submit">
                    <input type="submit" name="submit" id="submit" class="button-primary" value="<?php _e('Save New User ', 'EWD_FEUP') ?>"/>
            ...
            <script>
                jQuery(function($) { // onDomReady
                    ...
                    // reset handler that clears the form's radio buttons
                    $('form[name="newuser"] input:reset').click(function () {
                        $('form[name="newuser"]').find(':radio').removeAttr('checked').end();
                        return false;
                    });
                });
            </script>
        ...
        <?php if ($EWD_FEUP_Full_Version == "Yes") { ?>
            <div class="wrap">
                ...
                <form id="addtag" method="post"
                      action="admin.php?page=EWD-FEUP-options&Action=EWD_FEUP_AddUserSpreadsheet&DisplayPage=Users"
                    ...
                    <p class="submit"><input type="submit" name="submit" id="submit" class="button-primary"
                                             value="<?php _e('Add New Users', 'EWD_FEUP') ?>"/></p>
                </form>
            </div>
        ...
        <?php } else { ?>
            <div class="Info-Div">
                ...
                <div class="ewd-feup-full-version-explanation">
                    <?php _e("The full version of Front-End Only Users is required to use tags.", "EWD_FEUP"); ?><a
                        href="http://www.etoilewebdesign.com/front-end-users-plugin/"><?php _e(" Please upgrade to unlock this page!", 'EWD_FEUP'); ?></a>
                </div>
            </div>
        <?php } ?>
        ...
        <?php if ($EWD_FEUP_Full_Version == "Yes") { ?>
            <div class="wrap">
                ...
                <form method="post" action="admin.php?page=EWD-FEUP-options&Action=EWD_FEUP_ExportToExcel">
                    ...
                    <p class="submit"><input type="submit" name="Export_Submit" id="submit"
                                             class="button button-primary" value="Export to Excel"/></p>
                </form>
            </div>
        <?php } else { ?>
            <div class="Info-Div">
