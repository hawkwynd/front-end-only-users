...
function setLogoutShortcodeHelpHandler() {
	jQuery('.wp-editor-area').on('keyup', function(){
		if (jQuery(this).val().indexOf("[logout help") >= 0) {
			...
			AttributeString += "no_message='Yes' set to Yes to have no message appear on logout\n";
			AttributeString += "redirect_page='/logout' the page to redirect visitors after a successful logout\n";
			AttributeString += "]";
...
function setForgotPasswordShortcodeHelpHandler() {
	jQuery('.wp-editor-area').on('keyup', function(){
		if (jQuery(this).val().indexOf("[forgot-password help") >= 0) {
			...
			AttributeString += "reset_email_url='/confirm-forgot-password' the URL of your page with the confirm-forgot-password shortcode\n";
			AttributeString += "]";
